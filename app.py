...
def gerar_download_excel(df):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        for row_num, val in enumerate(df["SALDO SGM TÉCNICO"], start=1):
            if str(val).strip().lower() == "tem no saldo":
                worksheet.write(row_num, col_idx, val, formato_tem)
            elif str(val).strip().lower() in ["não tem no saldo", "nao tem no saldo"]:
...
# Título sem emoji para evitar erro de codificação
...
.kpi-box {
    background-color: #007acc; color: white; border-radius: 10px;
    padding: 20px 30px; flex: 1; text-align: center;
    box-shadow: 0 4px 6px rgb(0 0 0 / 0.1); font-family: 'Segoe UI';
}
...
st.dataframe(df_pendentes_display.style.applymap(destaque_saldo, subset=["SALDO SGM TÉCNICO"]), use_container_width=True)
