...
def carregar_dados():
    ...
    # Ordenar e pegar última inspeção por técnico + produto
    df_com_data = df_com_data.sort_values('Data_Inspecao', ascending=False)
    df_com_data_unicos = df_com_data.drop_duplicates(subset=[tecnico_col, produto_col], keep='first')

    # Incluir apenas pendentes que não têm inspeção
    chave = [tecnico_col, produto_col]
    df_sem_data_filtrado = df_sem_data.merge(
        df_com_data_unicos[chave], on=chave, how='left', indicator=True
    )
    df_sem_data_filtrado = df_sem_data_filtrado[df_sem_data_filtrado['_merge'] == 'left_only'].drop(columns=['_merge'])

    # Juntar resultado final
    df_resultado = pd.concat([df_com_data_unicos, df_sem_data_filtrado], ignore_index=True)
